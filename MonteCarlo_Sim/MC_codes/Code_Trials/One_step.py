...
def LorentzGamma(beta1, beta2, costheta, g_me): 
    ...
    if (E_approx == True):
        ...
        Dbeta = beta2 - beta1
        betaD = mp.mpf(Gamma1**2* Dbeta* (1 + Gamma1**2 * beta1 * Dbeta))
        g_me2 = mp.mpf(g_me*(1 + 0.5*betaD**2 - betae*betaD*costheta))
    
    else : 
        betaD = mp.mpf((beta2 - beta1) / (1 - beta1 * beta2))
        dGM = mp.mpf(1.0 / np.sqrt(1 - betaD**2))
        g_me2 = mp.mpf(dGM * g_me * (1 - betae * betaD * costheta))
