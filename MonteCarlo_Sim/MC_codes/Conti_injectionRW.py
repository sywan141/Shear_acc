...
output_dir = '/home/wsy/Acc_MC/Results/trial_Cinjection_RW/'
# 读取初始参数
...
def Single_Par(K):
    if K>= N_time:
        raise ValueError('The number of particles should be smaller than the time steps')
    ...
    while N_count >= 0:
        ...
        if poss >  1- np.exp(-dt/tau): 
            ...
            # 在无散射情况下，速度方向的改变只包括坐标系变换的部分，没有各向同性散射的影响
            beta1 = beta_dis(r) # 记录位移前的流速
            x,y,z = movement_e(u_cmv, beta2 ,dt, x, y, z) 
            r = np.sqrt(x**2 + y**2)
            ...
            theta_prev, phi_prev = Get_Angles(u_cmv) # 将方向修正到下个坐标系
