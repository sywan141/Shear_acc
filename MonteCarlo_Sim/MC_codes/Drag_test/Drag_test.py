...
def SA_Gamma(costheta_prev, alpha_prev, g_me, ba):
    ...
    cos_in = costheta_prev * costhetaM + sintheta_prev * sinthetaM * np.cos(alpha_in) # 喷流共动系下，磁云轴方向的入射角余弦
...
def Single_Par(K):
    ...
    # 临时随机化注入位置
    #poss = 1e-3 #np.random.uniform(eta,1)
    #r0 = poss*R_sh   
    r0 = 1e16
    ...
    gme_obs = LorentzGamma(beta2, beta_ini, -1 , g_me0)
    ...
    while (N_count>=0):
        ...
        if poss >  1- np.exp(-dt/tau): # 无散射分支
            ...
            beta1 = beta_dis(r, GM0, R_sh) # 记录当前位置
            x,y,z,dz_jet = movement_e(g_me,costheta_prev,alpha_prev,beta2,dt,x,y,z) # 向上一步变换后的方向移动
            r = np.sqrt(x**2 + y**2)
            ...
            # 没有出界则更新参数
            t_j += dt
            t_o += LorentzT(beta2,dz_jet,dt) # 位置更新前
        ...
        x,y,z,dz_jet = movement_e(g_me,costheta_prev,alpha_prev,beta2,dt,x,y,z)
        r = np.sqrt(x**2 + y**2)
