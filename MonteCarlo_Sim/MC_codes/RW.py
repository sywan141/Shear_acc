...
def Single_Par(K):
    ...
    beta1 = beta_dis(r0, GM0, R_sh) # 每次辐射前一步位置
    beta2 = beta_dis(r0, GM0, R_sh) # 辐射时当前位置
    beta_ini = 0 
    gme_obs = LorentzGamma(beta1, beta_ini, -1 , g_me0)
    ...
    while (N_count>=0):
        ...
        if poss >  1- np.exp(-dt/tau): # 无散射分支
            if Sh:
                g_me = LorentzGamma(beta1, beta2, costheta_prev, g_me)
            # 同步辐射能损
            if syn:
        ...
        # 散射分支，加入随机加速造成的散射方位角
        if Sh:
            g_me = LorentzGamma(beta1, beta2, costheta_prev, g_me) # 在beta2处发生散射
            # 直接生成新的移动方向
