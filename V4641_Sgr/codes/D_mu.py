...
def integrand_D(n, r_g, MU, MU_K, sign):
    ...
    z[valid] = (r_g * k_val[valid] * sqrt_term[valid]) / c
...
def Total_D(r_g):
    ...
    n_x, n_y = 2000, 2000  # Grid resolution
    ...
    integral = sum(np.sum(term) for term in terms) * delta_mu * delta_mu_k # add functions in all grids, four functions in all
    return integral

# Parallel computation setup
if __name__ == '__main__':
    r_g = np.logspace(-4, 2, 5000) / k_min
    with Pool() as pool:
        D_res = D_res = pool.map(Total_D, r_g)
