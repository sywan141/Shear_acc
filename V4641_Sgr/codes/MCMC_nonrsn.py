...
def log_prior(theta):
    ...
    if  (lgB_min<lgB<lgB_max) and (0<rate<1) and (0.3<beta<1.0) and (44.0<lgN<lgN_max) and (lgrho_min<lgrho<lgrho_max):
        return 0.0
    return -np.inf
...
#-------------------------------------------------------------------------------------------------------
    new_folder = "A_LorentzC_nonrsn_MFP_xi=1_%spc_limited_DIFFUSION_MODIFIED_%s_GalacticExtinction_q=%s_widerange"%(R,num,q)
...
#-------------------------------------------------------------------------------------------------------
    ...
    plt.savefig('/home/wsy/V4641_Sgr/PDF_files/'+'corner_%spc_LorentzC.pdf'%R, format="pdf", dpi=300, bbox_inches="tight") # save pdf results
    plt.show()
...
# plot chi2
    ...
    plt.savefig(dir_p + 'SYN_%spc_LorentzC.png'%R)
    plt.show()

    # save txt files
    file1 = np.column_stack((spectrum_energy_syn,Flux_best,sed_min,sed_max))
    np.savetxt(dir_data1+'SYN_%spc_spec_LorentzC.txt'%R,file1)
    ...
    plt.legend(frameon=False)
    plt.savefig(dir_t + 'IC_%spc_LorentzC.png'%R)
    plt.show()

    # save txt files
    file2 = np.column_stack((spectrum_energy_ic,IC_tot, IC_cmb, IC_fir, IC_nir, sed_min, sed_max))
    np.savetxt(dir_data2+'IC_%spc_spec_LorentzC.txt'%R,file2)
    ...
    np.savetxt(dir_t + 'Range_%spc_LorentzC.txt' % R, rows) # 1 sigma ranges
    np.savetxt(dir_t + 'Range_all_paras_%spc_LorentzC.txt' % R, stacked_param) # all paras in 1 sigma
