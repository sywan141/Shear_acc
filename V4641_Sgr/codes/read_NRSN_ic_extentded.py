...
from NAIMA_RAD import NAIMA_IC, NAIMA_SYN, SSEPL
...
def process_paras(paras, R):
    lgB, rate, beta, lgrho, lgK = paras
    ene_eV, n_eV = SSEPL(R, lgB, rate, beta, lgK, lgrho)
    sed_SYN = NAIMA_SYN(ene_eV, n_eV, lgB, beta, lgK)
    sed_IC = NAIMA_IC(ene_eV, n_eV, beta, lgK)[0]
    return sed_SYN, sed_IC

for i in R:
    flat_samples = np.loadtxt(datadir + 'ARadius_KDE_MFP_Cooling_free_FP_GYR_fixedLam_xi=1_%spc_limited_DIFFUSION_MODIFIED_0_GalacticExtinction_q=1.6666666666666667_widerange/'%i+
    'paras_lgB0=-0.427_rate=0.458_beta=0.773_lgrho=-31.756_lgK1=66.383_n=60000_weighted.txt')
    chi2_samples = np.loadtxt(datadir+'ARadius_KDE_MFP_Cooling_free_FP_GYR_fixedLam_xi=1_%spc_limited_DIFFUSION_MODIFIED_0_GalacticExtinction_q=1.6666666666666667_widerange/'%i+
    'chi2_lgB0=-0.427_rate=0.458_beta=0.773_lgrho=-31.756_lgK1=66.383_n=60000_weighted.txt')
    ...
    # record ic radiation
    lgB_best,rate_best,beta_best,lgrho_best,lgK_best = theta_best
    ...
    ene_best,n_best = SSEPL(i,lgB_best,rate_best,beta_best,lgK_best,lgrho_best)
    IC_tot, IC_cmb, IC_fir, IC_nir, IC_obs= NAIMA_IC(ene_best,n_best,beta_best,lgK_best)
    ...
    np.savetxt(tar_dir+'IC_%spc_spec_nrsn.txt'%i,filei)